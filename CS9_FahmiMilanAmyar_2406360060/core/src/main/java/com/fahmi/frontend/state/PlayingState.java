...
import com.fahmi.frontend.strategies.*;
...
//            obstacle.render(shapeRenderer);
    ...
    private void spawnCoins() {
        ...
        if (!coinPatterns.isEmpty()) {
            ...
            List<Coin> spawnedCoins = pattern.spawn(coinFactory, ground.getTopY(), spawnX, screenHeight);

            for (Coin coin : spawnedCoins) {
                coinFactory.getActiveCoins().add(coin);
            }
        }
    }
